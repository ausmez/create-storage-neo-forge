...
public class PasserBlockScenes {
    public static void intro(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.addKeyframe();
        scene.overlay.showControls((new InputWindowElement(util.vector.blockSurface(srcBox1, Direction.NORTH), Pointing.RIGHT)).withItem(sand), 20);
        scene.world.modifyBlockEntity(srcBox1, StorageBoxEntity.class, (t) -> t.getItemHandler().setStackInSlot(0, sand.copyWithCount(12)));
        scene.world.modifyBlock(srcBox1, (s) -> ModPonder.STORAGE_BOX.getDefaultState().setValue(STORAGE_USED, EnumProperties.StorageUsed.HAS_ITEMS), false);
        scene.idle(40);
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (i == 11)
                scene.world.modifyBlock(srcBox1, (s) -> ModPonder.STORAGE_BOX.getDefaultState().setValue(STORAGE_USED, EnumProperties.StorageUsed.EMPTY), false);
            scene.idle(15);
        }
