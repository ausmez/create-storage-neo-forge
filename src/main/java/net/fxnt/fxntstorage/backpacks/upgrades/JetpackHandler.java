...
public class JetpackHandler {
    ...
    public void execute() {
        if (player != null) {
            ...
            if (player.getPersistentData().getBoolean("fxntJetpackFlying")) { // Space bar has been pressed
                isJumping = true;
    ...
    public void toggleHover() {
        if (player.onGround()) {
            endHovering(false);
            player.getPersistentData().putBoolean("fxntJetpackHover", false);
            return;
        }
        if (!isHovering) {
            startHovering(true);
        ...
        player.getPersistentData().putBoolean("fxntJetpackHover", isHovering);
    }
    ...
    public void updateJetpackMovement() {
        ...
        double forward = player.getPersistentData().getFloat("fxntJetpackforward");
        double strafe = player.getPersistentData().getFloat("fxntJetpackleft");
    ...
    private double calculateHorizontalSpeed() {
        ...
        double defaultPlayerSprintSpeed = 0.21;
...
//        ModNetwork.sendToPlayer((ServerPlayer) player, new VisualJetpackAirPacket(-1));
    ...
    public static void processPlayerInputPacket(ServerPlayer player, float forward, float left) {
        if (player != null) {
            player.getPersistentData().putFloat("fxntJetpackforward", forward);
            player.getPersistentData().putFloat("fxntJetpackleft", left);
        }
    }
