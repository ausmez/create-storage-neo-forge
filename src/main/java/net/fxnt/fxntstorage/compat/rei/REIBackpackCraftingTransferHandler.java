...
public class REIBackpackCraftingTransferHandler implements SimpleTransferHandler {

    @Override
    public ApplicabilityResult checkApplicable(Context context) {
        boolean applicable = context.getMenu() instanceof BackpackMenu menu
                && menu.isCraftingPanelReady()
                && context.getDisplay().getCategoryIdentifier() == BuiltinPlugin.CRAFTING
    ...
    public Iterable<SlotAccessor> getInputSlots(Context context) {
        if (!(context.getMenu() instanceof BackpackMenu menu)) return List.of();
        return menu.getCraftingMatrixSlots().stream().map(SlotAccessor::fromSlot).toList();
    }
    ...
    public Iterable<SlotAccessor> getInventorySlots(Context context) {
        ...
        // The open backpack's item section
        if (context.getMenu() instanceof BackpackMenu menu) {
