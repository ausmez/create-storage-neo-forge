    ...
    public Result handle(Context context) {
        ...
        if (missing.isEmpty()) {
            if (!context.isActuallyCrafting()) {
                ...
                Container inv = new SimpleContainer((ItemStack) stacks.get(0).get(0).getValue());
                List<StonecutterRecipe> recipes = client.level.getRecipeManager().getRecipesFor(RecipeType.STONECUTTING, inv, client.level);

                for (int i = 0; i < recipes.size(); i++) {
                    if (recipes.get(i).getId().equals(display.getDisplayLocation().get())) {
                        client.gameMode.handleInventoryButtonClick(client.player.containerMenu.containerId, i);
                        break;
                    }
                }
